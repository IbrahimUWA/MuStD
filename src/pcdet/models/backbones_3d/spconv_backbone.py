...
from det3d.models.readers.pillar_encoder import PillarFeatureNet,PFNLayer,PillarNet, PillarFeatureNet2
...
def voxel_indices_to_pillar_indices(voxel_indices):
    # Assuming voxel_indices is a tensor of shape (num_points, 3)
    # where each row is a 3D voxel index (x, y, z)

    new_uv = voxel_indices.clone().int()
    # Calculate the corresponding 2D pillar indices
    pillar_indices = voxel_indices[:, 0:2]  # Keep only the x and y coordinates

    # Assuming grid_size is a tuple (grid_width, grid_height)
    # You can adjust the grid size based on your specific mapping
    # Ensure the pillar indices are within the grid boundaries
    new_uv[:, 2] = 1
    new_uv[:, 0] = torch.clamp(pillar_indices[:, 0], min=0, max=1400)
    new_uv[:, 1] = torch.clamp(pillar_indices[:, 1], min=0, max=600)
    return new_uv[:,0:3]
...
class NRConvBlock(nn.Module):
    ...
    def __init__(self, input_c=16, output_c=16, stride=1, padding=1, indice_key='vir1', conv_depth=False):
        ...
        self.d2_conv3 = block2d(c2 // 2,
                                c2 // 2,
                                3,
                                norm_fn=norm_fn,
                                padding=1,
                                indice_key=('subm5' + indice_key))

        self.d2_conv4 = block2d(c2 // 2,
                                c2 // 2,
                                3,
                                norm_fn=norm_fn,
                                padding=1,
                                indice_key=('subm6' + indice_key))

        self.output_conv = block2d(c2 ,
                                c2 // 2,
                                3,
                                norm_fn=norm_fn,
                                padding=1,
                                indice_key=('subm7' + indice_key))
    ...
    def forward(self, sp_tensor, batch_size, calib, stride, x_trans_train, trans_param):
        ...
        pillars = voxel_indices_to_pillar_indices(d3_feat2.indices)
        ...
        d2_sp_tensor2 = spconv.SparseConvTensor(
            features=d3_feat2.features,
            indices=pillars.contiguous().int(),
            spatial_shape=[1600, 600],
            batch_size=batch_size
        )
        ...
        # ten = d2_sp_tensor2.contiguous()
        pillar1 = self.d2_conv3(d2_sp_tensor2)
        # print(pillar1.features.shape, "pillar1")
        pillar2 = self.d2_conv4(pillar1)
        d2_feat2 = replace_feature(d2_feat2, torch.cat([pillar2.features, d2_feat2.features], -1))
        out = self.output_conv(d2_feat2)
        d3_feat3 = replace_feature(d3_feat2, torch.cat([d3_feat2.features, out.features], -1))
        return d3_feat3
...
class VirConv8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size,  **kwargs):
        ...
        self.pillarfeatures = PillarFeatureNet(13, [64, 64],[0.05, 0.05, 0.05],[0, -40, -3, 70.4, 40,1])
        self.pillar = PillarNet(9,[0.05, 0.05, 0.05],[0, -40, -3, 70.4, 40,1])
        self.sparsenet = SparseResNet([2, 2, 2, 2],  [2, 2, 2, 1], [64, 128, 256, 256], 64)
    ...
    def forward(self, batch_dict):
        ...
        feat_max, coords, grid_size = self.pillarfeatures(pts)
        spareseout = self.sparsenet(feat_max, coords, grid_size)

        #print("spareseout", spareseout.shape)
        ...
        if self.training:
            ...
            for i in range(rot_num):
                ...
                # print(this_out.dense().shape,"Dense features")
        ...
        for i in range(rot_num):
            ...
            if self.model_cfg.get('MM', False):
                ...
                # print(newvoxel_features.shape, "newvoxel_features")
                # print(newvoxel_coords.shape, "newvoxel_coords")
                ...
                # print(newinput_sp_tensor.features.shape, "newinput_sp_tensor")
                if self.training:
                ...
                # if self.training:
                #     transed = self.x_trans_train.backward_with_param({'points': pts[:, 1:4],
                #                                                       'transform_param': trans_param[i]})
                #     cur_pts = transed['points']
                #     pts[:,1:4] = cur_pts
                #     feat_max, coords, grid_size = self.pillarfeatures(pts)
                #     spareseout1 = self.sparsenet(feat_max, coords, grid_size)
                #     spareseout = spareseout1 + spareseout
                newx_conv1 = self.vir_conv1(newinput_sp_tensor, batch_size, calib, 1, self.x_trans_train, trans_param)
                # print(newx_conv1.features.shape, "newx_conv1")
