...
# from pcdet.models.backbones_2d.parse_config import *
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape,"output after encoding")
        ...
        x = self.conv2(x)
        x = F.relu(x)
        # print(x.shape, "output after two cnns")
        #         x = F.pad(x, (4,4,4,4)) # no longer needed
        x = self.conv3(x)
        x = F.relu(x)
        # print(x.shape, "output after 3 cnns")
        x = self.conv4(x)
        x = F.relu(x)
        # print(x.shape, "output after 4 cnns")
        x = self.conv5(x)
        x = F.relu(x)
        x = self.conv6(x)
        # print(x.shape, "output after 6 cnns")
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, num_iterations, hid_dim):
        ...
        self.upsamplayers = nn.Sequential(nn.Upsample(size=(200, 176), mode='nearest')
        )
    ...
    def forward(self, image):
        ...
        # print(x.shape,"decoder shape")
        # `x` has shape: [batch_size*num_slots, width, height, num_channels+1].

        # Undo combination of slot and batch dimension; split alpha masks.
        _, masks = x.reshape(image.shape[0], -1, x.shape[1], x.shape[2], x.shape[3]).split([3, 1], dim=-1)
        # `recons` has shape: [batch_size, num_slots, width, height, num_channels].
        # `masks` has shape: [batch_size, num_slots, width, height, 1].

        # Normalize alpha masks over slots.
        masks = nn.Softmax(dim=1)(masks)
        masks = torch.squeeze(masks,4)
        masks = masks.permute(0, 1, 2,3)
        masks = self.fc3(masks)
        masks = masks.permute(0, 3,1, 2)
        masks = self.upsamplayers(masks)
...
if __name__ == '__main__':
    # slot =SlotAttentionAutoEncoder().cuda()
    # print(slot)

    slot = SlotAttentionAutoEncoder([375,1242], 64, 5,64).cuda()


    # def __init__(self, resolution, num_slots, num_iterations, hid_dim):

    # module_defs = parse_model_config('/media/ibrahim/21b7344f-7f8b-47fb-8de7-5199b164b720/VirConv/pcdet/models/backbones_2d/yolov3-kitti.cfg')
    # hyp, model= create_modules(module_defs)
    #
    #
    # darknet = Darknet('/media/ibrahim/21b7344f-7f8b-47fb-8de7-5199b164b720/VirConv/pcdet/models/backbones_2d/yolov3-kitti.cfg')
    # darknet.load_weights('/media/ibrahim/21b7344f-7f8b-47fb-8de7-5199b164b720/VirConv/pcdet/models/backbones_2d/best_weights_kitti.pth')
    # print(darknet.module_list)
    image = torch.rand(2,3,375, 1242).cuda()
    masks = slot(image)
    # print(recon_combined.shape,"recon_combined")
    # print(recons.shape,"recons")
    print(masks.shape,"masks")
    # print(slots.shape,"slots")
    # img = darknet.forward(image)

    # print(img.shape)

    # orch.Size([2, 256, 200, 176])
